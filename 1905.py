...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def detailContent(self,array):
		...
		if aid.isdigit()==False:			
			...
			if self.custom_RegexGetText(url,r'/(film)/',1)!='':
				...
				if url=='':
					url=self.custom_RegexGetText(HtmlTxt,r'(https*://[a-z0-9.]*1905\.com/vod/play/\d+\.shtml)',1)
			if len(self.custom_RegexGetText(url,r'(vip.1905)',1))>3:
				...
				if title=='':
					title=self.custom_RegexGetText(HtmlTxt,r'property="og:title"\s*content="(.+?)"',1)
				pic=self.custom_RegexGetText(HtmlTxt,r'<img class="poster" src="(.+?)"',1)
				if pic=='':
					pic=self.custom_RegexGetText(HtmlTxt,r'property="og:image"\s*content="(.+?)"',1)
				content=self.custom_RegexGetText(HtmlTxt,r'<p>(.+?)</p>',1)
				if content=='':
					content=self.custom_RegexGetText(HtmlTxt,r'property="og:description"\s*content="(.+?)"',1)
	...
	header = {
		# 1905网PC端已加WAF防护(返回403 JS挑战),移动端UA可正常访问
